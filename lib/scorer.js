 ...
 *
 * Deduplication: one chip per outlet. When the same source contributes multiple
 * matching articles (e.g. The Guardian #1, #2, #3), keep only the best-ranked
 * member so the UI never shows repeated source names.
 */
export function buildSourceChips(cluster) {
  // Deduplicate: one entry per outlet label, keeping the lowest rank (best position)
  const best = new Map(); // label → member
