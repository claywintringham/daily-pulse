...
//      N expands from 3 up to the highest rank present in the data; stops
//      pool reaches `cap` stories (caller passes display cap so we don't
//      collect more than we can show).
...
//   5. Fallback (local bucket only): if the pool is still empty after rank 6,
//      one additional pass at rank 6 with minVotes = 1 guarantees at least
//      one story from a single reputable local outlet.

/**
 * Score and rank clusters within one bucket (international or local).
 * @param {object[]} clusters
 * @param {'international'|'local'} bucket
 * @param {number} cap  Stop expanding rank once we have this many stories.
 *                      Defaults to Infinity (collect everything).
 */
export function scoreClusters(clusters, bucket, cap = Infinity) {
  const bucketClusters = clusters.filter(c => c.bucket === bucket);

  const allScored = [];
  const seen = new Set();

  // Find the highest rank present in this bucket so the loop has a natural
  // ceiling — no need for a hard-coded number.
  const maxPossibleRank = bucketClusters.reduce(
    (hi, c) => Math.max(hi, ...c.members.map(m => m.rank ?? 0)),
    0
  );

  // Expand the rank window one step at a time (starting at 3 to require at
  // least some editorial prominence) until the cap is filled or we've
  // exhausted every rank that actually exists in the data.
  for (let maxRank = 3; maxRank <= maxPossibleRank; maxRank++) {
    ...
    // Stop expanding once we have enough stories to fill the display cap
    if (allScored.length >= cap) break;
  }

  // Fallback: nothing qualified even at the loosest real rank — genuine news
  // lull or very thin source coverage. Do one pass with a relaxed single-vote
  // threshold so we don't silently return zero stories.
  if (allScored.length === 0) {
    const fallback = qualify(bucketClusters, maxPossibleRank, 1);
    for (const c of fallback) {
      if (seen.has(c.id)) continue;
      seen.add(c.id);
      allScored.push({
        ...c,
        baseScore:         computeBaseScore(c, maxPossibleRank),
        bonusScore:        computeBonusScore(c),
        qualificationRank: maxPossibleRank + 1, // sentinel: fallback tier
      });
    }
  }
 ...
 */
function qualify(clusters, maxRank, minVotes = 2) {
  return clusters.filter(cluster => {
    ...
    return effectiveVotes >= minVotes;
  });
}
