...
//      N expands through [3, 4, 5, 6]; expansion stops as soon as the scored
...
export function scoreClusters(clusters, bucket, cap = Infinity) {
  ...
  // Fallback for local bucket: if every rank level came up empty, do one
  // final pass at rank 6 with a relaxed single-vote threshold so a genuine
  // news lull doesn't silently produce zero local stories.
  if (allScored.length === 0 && bucket === 'local') {
    const fallback = qualify(bucketClusters, 6, 1);
