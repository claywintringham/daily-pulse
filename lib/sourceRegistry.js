...
export const SOURCES = [
...

  {
    ...
    phase:   2,
    status:  'active',
    skipDom:  true, // always returns 0 DOM items; goes directly to RSS
  ...
  // HKET (香港經濟日報): Chinese-language economic daily. Hard paywall — article
  // content is behind a subscription. RSS feeds exist but return empty channels.
  // The homepage DOM IS accessible and exposes full headlines in <a title="...">
  // attributes on inews.hket.com article links. Treated as paywalled (headline-only,
  // 🔒 chip). Headlines translated by translateHeadlines() before Jaccard clustering.
  {
    id:              'hket',
    label:           'HKET',
    bucket:          'local',
    ...
    rssUrl:          null,    // RSS feeds exist but are empty shells
    renderMode:      'fetch',
  ...
  // ONCC (東網 / on.cc): high-traffic HK Chinese-language news portal.
  // No traditional RSS feed. Uses a Google News sitemap at hk.on.cc/sitemap.xml
  // which covers all sections. Filtered to /cnt/news/ (HK local) and /cnt/intnews/
  // (international), excluding /cnt/entertainment/ and /cnt/sport/.
  // Headlines translated to English by translateHeadlines() before clustering.
  {
    id:                  'oncc',
    label:               'On.cc',
    bucket:              'local',
  ...
  // Ming Pao (明報): Chinese-language broadsheet. Soft paywall — RSS is fully
  // accessible and returns 15 top news headlines. Headlines are translated to
  // English by translateHeadlines() before Jaccard clustering so they can
  // match against English-language sources covering the same HK stories.
  {
    id:              'mingpao',
    label:           'Ming Pao',
    bucket:          'local',
  ...
  // WSJ: hard paywall. Public RSS feeds via Dow Jones are the reliable path —
  // the homepage is heavily JS-rendered. World News RSS returns ~15 headlines
  // across global topics. DOM selectors kept as best-effort fallback.
  {
    id:          'wsj',
    label:       'WSJ',
    bucket:      'international',
  ...
  // the Markets/World News feed. DOM selectors kept as a best-effort fallback.
  {
    ...
    phase:       3,
    status:      'active',
    skipDom:     true, // heavily JS-rendered, always returns 0 DOM items; goes directly to RSS
