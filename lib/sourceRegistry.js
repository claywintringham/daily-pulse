...
//
// Content strategy flags (handled by lib/enricher.js):
//   rssDescriptionOnly:true — isPaywalled source with useful RSS descriptions.
//                             Phase 1 (RSS description) is used with a 25-word
//                             threshold; Phase 2 article fetch is skipped.
...
export const SOURCES = [
...

  {
    ...
    label:   'HKFP',
    bucket:  'local',
    // Articles are freely accessible — isPaywalled was previously true only because
    // donation prompts polluted summaries. JUNK_RE now handles those; Phase 2
    // direct article fetch is re-enabled.
    isPaywalled: false,
    phase:   1,
    status:  'active',
...

  {
    ...
    status:              'active',
    needsTranslation:    true,
    // on.cc uses a Google News sitemap (has <news:title> tags) — fetchSitemapItems works.
    // Include only general news and international news paths; exclude sport/entertainment/finance.
    entryUrl:            'https://hk.on.cc',
    sitemapUrl:          'https://hk.on.cc/sitemap.xml',
    includeUrlPatterns:  ['/cnt/news/', '/cnt/intnews/'],
    excludeUrlPatterns:  ['/cnt/entertainment/', '/cnt/sport/', '/cnt/finance/', '/cnt/weather/', '/bknrac/'],
    maxRank:             10,
  },
  ...
  // rssDescriptionOnly:true — Phase 1 (RSS desc, 25-word threshold) allowed;
  // Phase 2 article fetch skipped (would hit paywall).
  {
    id:                  'nyt',
    label:               'NYT',
    bucket:              'international',
    isPaywalled:         true,
    rssDescriptionOnly:  true,
    phase:               3,
    status:              'active',
  ...
  // Sky News: UK broadcaster, free, React SPA — RSS description only (Phase 2 skipped).
  {
    id:      'skynews',
    label:   'Sky News',
    bucket:  'international',
  ...
  // CBC News: Canadian public broadcaster, free, Next.js SSR — Phase 2 direct fetch works.
  {
    id:      'cbcnews',
    label:   'CBC News',
    bucket:  'international',
  ...
  // The Times (UK): hard paywall — custom SSR-JSON adapter extracts headlines +
  // publishedTime directly from Next.js page source (own RSS is auth-gated).
  // Clustering signal only; no article content ever fetched.
  {
    id:          'thetimes',
    label:       'The Times',
    bucket:      'international',
  ...
  // ABC News (Australia): public broadcaster, free, Next.js SSR — Phase 2 direct fetch works.
  {
    id:      'abcau',
    label:   'ABC News (AU)',
    bucket:  'international',
    ...
    status:  'active',
    rssFirst: true,
    entryUrl: 'https://www.abc.net.au/news',
    rssUrl:   'https://www.abc.net.au/news/feed/45910/rss.xml',
    renderMode: 'fetch',
    containers: ['[data-component="Card"]', '.docList', 'main'],
    itemSelectors: [
      'h3 a[href*="abc.net.au"]',
      'h2 a[href*="abc.net.au"]',
      'a[href*="/news/20"]',
    ],
    excludeSelectors: ['nav', 'footer', '.ad', '[class*="promo"]'],
    maxRank: 7,
