...
// Paywalled sources are skipped entirely — their content is behind a paywall
// and their RSS descriptions are often truncated or contain promotional text.
...
export async function enrichWithArticleContent(clusters, { concurrency = 8, useFirecrawl = false } = {}) {
  for (let i = 0; i < clusters.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async c => {
      ...
      // RSS descriptions are often truncated or promotional (e.g. HKFP donation prompts).
      ...
      for (const m of sorted) {
        ...
        // Also run JUNK_RE to reject donation prompts, membership pitches, etc.
        if (m.description) {
          const cleaned = sanitiseExtract(m.description).replace(/\s+/g, ' ').trim();
          if (wc(cleaned) >= MIN_ARTICLE_WORDS && !JUNK_RE.test(cleaned)) text = trimToWords(cleaned);
        }

        // Phase 2: direct article fetch
        ...
        // Phase 2b: TVB inews-api fallback — when description is null, fetch the
        // individual article via the same internal API the adapter uses for headlines.
        // This is the best available method for TVB Pearl/News when a.desc is absent.
        if (!text && m.articleUrl && m.articleUrl.includes('news.tvb.com')) {
