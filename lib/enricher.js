...
// sorted longest-first into c.articleExcerpts[].
//
// Phase 1 minimum word counts:
//   AP, RTHK, TVB Pearl, TVB News → 60 words (broadcast/API copy is authoritative even when brief)
//   All others → 120 words
//
// Guardian API (Phase 3) is only used for international stories — it does not
// cover HK local news and produces false positives for local queries.

const MIN_ARTICLE_WORDS = 120;
const wc = text => (text ? text.trim().split(/\s+/).length : 0);

// Patterns that indicate the text is NOT article content — paywall, donation, navigation.
...
// Rejects text that doesn't look like the start of a real news article.
// Catches social widget labels, URL fragments, and byline/metadata openers.
function looksLikeProse(text) {
  const t = text.trim();
  if (!t) return false;
  // Reject URL fragments (token with domain/path pattern near start)
  if (/^\S*\.[a-z]{2,}\/\S*/i.test(t)) return false;
  // Reject if starts with a lowercase letter (widget labels, abbreviations)
  if (/^[a-z]/.test(t)) return false;
  // Reject known garbage openers: bylines, datelines, ad markers
  ...
  for (let i = 0; i < clusters.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async c => {
      ...
      for (const m of sorted) {
        ...
        if (m.description) {
          const cleaned = sanitiseExtract(m.description).replace(/\s+/g, ' ').trim();
          const minWords = (m.sourceId === 'ap' || m.sourceId === 'rthk' || m.sourceId === 'tvbpearl' || m.sourceId === 'tvb') ? 60 : MIN_ARTICLE_WORDS;
            ...
            if (idMatch) {
              ...
              try {
                ...
                if (tvbRes.ok) {
                  ...
                  const tvbMinWords = 60; // TVB articles are concise broadcast-style
                  if (body && wc(body) >= tvbMinWords) text = trimToWords(sanitiseExtract(body));
                }
              } catch { /* non-fatal */ }
            }
      ...
      // Phase 3: Guardian API as cluster-level fallback (international only —
      // Guardian does not cover HK local news and produces false positives)
