...
export function sanitiseExtract(text) {
  return text
    ...
    // Broadcast-promo filler: TV/streaming segments that leak into article
    // descriptions. Strip one sentence at a time (non-greedy, bounded).
    .replace(/\bFor more[,:]?\s+[^.]*?\bjoins?\b[^.]*?\b(?:to discuss|for (?:a )?(?:discussion|analysis))\b[^.]*\.?/gi, '')
    .replace(/\bTune in\b[^.]*\.?/gi, '')
    .replace(/\bStay with us\b[^.]*\.?/gi, '')
    .replace(/\bWatch (?:the full|live|more)\b[^.]*\.?/gi, '')
    .replace(/\bComing up (?:next|on [A-Z][A-Za-z]+)\b[^.]*\.?/gi, '')
    .replace(/\bUp next\b[^.]*\.?/gi, '')
    .replace(/\s{2,}/g, ' ').trim();
}
