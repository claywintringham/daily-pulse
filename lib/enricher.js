...
export function looksLikeNavJunk(text) {
  if (!text) return true;
  const t = text.trim();
  // Short fragments are likely UI chrome
  if (t.length < 40) return true;
  // Starts with a URL — definitely not prose
  if (/^\S*\.[a-z]{2,}\/\S*/i.test(t)) return true;
  // High density of nav-menu keywords near the start
  const sample = t.slice(0, 300).toLowerCase();
  const NAV_WORDS = ['home', 'news', 'sport', 'business', 'technology', 'menu',
    'skip to', 'navigation', 'subscribe', 'sign in', 'log in', 'watch live',
    'breaking news', 'latest news', 'share'];
  const hits = NAV_WORDS.filter(w => sample.includes(w)).length;
  ...
  for (let i = 0; i < clusters.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async c => {
      ...
      for (const m of sorted) {
        ...
        // Word-count thresholds by source type:
        //   rssDescriptionOnly (NYT)        → 25 words (short but substantive summaries)
        //   concise-authoritative sources   → 60 words
        //   all others                      → 120 words
        if (m.description) {
          const cleaned = sanitiseExtract(m.description).replace(/\s+/g, ' ').trim();
        ...
        // Skipped for rssDescriptionOnly sources (paywalled — article fetch would hit paywall).
        ...
        if (text && looksLikeProse(text) && !looksLikeNavJunk(text)) {
          excerpts.push(text);
