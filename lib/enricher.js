...
// per cluster (one per source, ≥120 words each), sorted longest-first, and
// stores them as c.articleExcerpts[]. summarizeClusters() in llm.js uses all
// of them so Gemini can draw on multiple sources for 60-80 word summaries.
...
export function sanitiseExtract(text) {
  return text
    ...
    // Strip site-navigation fragments that Readability sometimes includes
    ...
    // Strip embedded URLs and advertisement labels that slip through Readability
    .replace(/https?:\/\/\S+/gi, '')
    .replace(/\bp\.dw\.com\/\S*/gi, '')
    .replace(/\bAdvertisement\b/g, '')
    // Strip article metadata embedded in body text (SCMP-style: read-time, byline, timestamps)
...
// Still exported for use by pickSubstantiveExcerpt / enrichAndFilterClusters
...
// ── Main export ───────────────────────────────────────────────────────────────
// Collects up to 5 substantive article bodies per cluster (one per free source,
// ≥120 words each), sorted longest-first into c.articleExcerpts[].\n//
// Article fetching uses the best method for each source type:
//   TVB Pearl / TVB News  → inews-api by article ID (HTML fetch fails on SPA)
//   NBC, CBS, Fox, FoxBiz → Firecrawl (HTML fetch always blocked)
//   All others            → direct HTML fetch + Readability
// Paywalled sources are skipped entirely.
//
// Phase 1 minimum word counts:
//   AP, RTHK  → 60 words (wire/broadcast copy is authoritative even when brief)
//   All others → 120 words

const MIN_ARTICLE_WORDS = 120;
const wc = text => (text ? text.trim().split(/\s+/).length : 0);

// Patterns that indicate the text is NOT article content — it's a paywall gate,
// donation prompt, cookie notice, or site-navigation fragment.
...
export async function enrichWithArticleContent(clusters, { concurrency = 8, useFirecrawl = false } = {}) {
  for (let i = 0; i < clusters.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async c => {
      if (c.articleExcerpts?.length) return; // already enriched

      // Best free article URL for the "learn more" link
      ...
      // Process free members only, in source-priority order (AP > Reuters > BBC etc.)
      // Paywalled sources are excluded — their content is inaccessible and their
      // RSS descriptions are often truncated or promotional.
      const sorted = membersInPriorityOrder(c.members ?? []);

      for (const m of sorted) {
        if (excerpts.length >= 5) break;
        if (seenSources.has(m.sourceId)) continue;
        if (m.isPaywalled) continue; // paywalled sources never contribute article content

        let text = null;

        // Phase 1: RSS / API description field (instant — already in memory)
        // AP and RTHK produce authoritative wire/broadcast copy that is often
        // 50-80 words — still substantive. Lower their threshold accordingly.
        if (m.description) {
          const cleaned = sanitiseExtract(m.description).replace(/\s+/g, ' ').trim();
          const minWords = (m.sourceId === 'ap' || m.sourceId === 'rthk') ? 60 : MIN_ARTICLE_WORDS;
          if (wc(cleaned) >= minWords && !JUNK_RE.test(cleaned)) text = trimToWords(cleaned);
        }

        // Phase 2: source-specific best method for article body.
        // Each branch uses only the method that actually works for that source —
        // no wasted attempts on methods that reliably fail.
        if (!text && m.articleUrl && !SKIP_URL_RE.test(m.articleUrl)) {
          ...
          if (m.articleUrl.includes('news.tvb.com')) {
            ...
            // Direct HTML fetch always fails (Next.js SPA). Phase 1 description
            // comes from the category listing and is sometimes absent — the
            // individual article endpoint is the authoritative source.
            ...
            if (idMatch) {
              ...
              try {
                ...
                if (tvbRes.ok) {
                  ...
                  if (body && wc(body) >= MIN_ARTICLE_WORDS) text = trimToWords(sanitiseExtract(body));
                }
              } catch { /* non-fatal */ }
            }
            ...
            // Direct HTML fetch always returns blocked/empty content for these.
            if (useFirecrawl) {
              const fc = await fetchWithFirecrawl(m.articleUrl);
      ...
      // Phase 3: Guardian API as cluster-level fallback
      if (excerpts.length < 2 && !seenSources.has('guardian')) {
      ...
      // Sort longest first so Gemini gets the most content-rich articles first
