...
//   2. Pass 1 — descriptions: iterate the sorted list and collect up to 3
//      members whose RSS descriptions pass the quality gate.
//   3. Pass 2 — article bodies: if Pass 1 yields < 3, fetch each remaining
//      member's article URL (direct, with Firecrawl as last resort for
//      bot-blocked domains) and collect up to 3 passing article bodies.
//   4. If every member fails both passes, leave articleExcerpts empty —
...
//   • Not HTML nav junk (menu chrome, language toggles, three+ distinct nav labels)
...
// Menu/nav vocabulary — any single hit is fine on real articles, but three
// or more distinct labels clustered together almost certainly means the
// input is HTML navigation chrome rather than article content.
const NAV_LABEL_RE =
...
/**
 * Detect RSS descriptions that are actually HTML navigation chrome rather
 * than article content. These typically pass word/token counts trivially
 * (nav menus have plenty of words) but convey no story information.
 *
 * Exported for reuse in llm.js (fallback summary validation).
 */
...
export function passesQualityGate(text, headline) {
  ...
  // HTML/nav junk check — reject nav chrome even if it meets word/token counts
...
/**
 * For each cluster, attempt to populate `articleExcerpts` (array) using the
 * source-iteration quality gate described at the top of this file.
 *
 * Mutates clusters in place:
 *   - On success: sets `articleExcerpts` (array of up to 3), `articleExcerpt` (first
 *     for backward compat), and (if previously unset) `_learnMoreUrl`.
 *   - On total failure: leaves `articleExcerpts` empty. The caller is
...
export async function enrichWithArticleContent(clusters, { concurrency = 8, useFirecrawl = false } = {}) {
  ...
  for (let i = 0; i < clusters.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async c => {
      ...
      // ── Pass 1: collect up to 3 passing RSS descriptions in priority order ─
      const excerpts = [];
      ...
      if (excerpts.length) {
        ...
        c.articleExcerpt  = excerpts[0]; // kept for backward compat w/ fallback paths
      ...
      // ── Pass 2: fetch article bodies; collect up to 3 in priority order ───
      const fetched = [];
      ...
      // Both passes failed — articleExcerpts stays empty; caller drops this cluster.
