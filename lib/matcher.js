...
//
// When rssUrl is null (e.g. thetimes custom adapter sets publishedAt directly),
// enrichWithRss preserves any existing publishedAt rather than overwriting with null.
...
export async function enrichWithRss(items, rssUrl) {
  ...
  if (!rssItems.length) {
    // Preserve any publishedAt already set by a custom adapter (e.g. thetimes SSR-JSON).
    // Only null out publishedAt when it wasn't already populated.
    return items.map(i => ({ ...i, publishedAt: i.publishedAt ?? null, matchConfidence: 'low' }));
  }
  ...
  return items.map(item => {
    ...
    return { ...item, publishedAt: item.publishedAt ?? null, matchConfidence: 'low' };
  });
}
