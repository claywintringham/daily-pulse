  ...
  try {
    ...
    while ((m = itemRe.exec(xml)) !== null) {
      ...
      const description = descMatch ? (() => {
        const raw = descMatch[1].replace(/<[^>]+>/g, ' ').replace(/&[a-z#0-9]+;/gi, ' ').replace(/\s+/g, ' ').trim();
  ...
  return items.map(item => {
    ...
    if (exactUrl) {
      return {
        ...
        description:     item.description ?? (exactUrl.description || null),
    ...
    if (bestScore >= 0.40) {
      return {
        ...
        description:     item.description ?? (best.description || null),
        matchConfidence: 'high',
      };
    }
