...
const HK_SIGNALS = [
  ...
  // ── Known HK outlet URL fragments (present in articleUrl) ────────────────
  'rthk.hk', 'hkfp.com', 'thestandard.com.hk', 'scmp.com',
  'hk01.com', 'mingpao.com', 'hket.com', 'on.cc',
];

/**
 * Return true if the combined headline + URL text of a cluster's members
 * contains recognisable Hong Kong geographic or institutional signals.
 */
function isHongKongStory(members) {
  const text = members
    .map(m => `${m.title} ${m.articleUrl ?? ''}`)
    .join(' ')
    .toLowerCase();
