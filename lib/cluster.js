...
import { clusterHeadlines }       from './llm.js';
 ...
 * }
 */
export async function buildClusters(adapterResults) {
  ...
  let groups; // array of arrays of indices into allItems

  try {
    groups = await clusterHeadlines(allItems);
    ...
    groups = jaccardCluster(allItems);
  }

  // Convert groups to member arrays, enforcing one-item-per-source
  const rawClusters = groups.map(indices => {
    const members = [];
    const seenSources = new Set();
    // Sort by rank so the best-ranked item from each source wins
  ...
  return rawClusters.map((members, idx) => {
...
// Patterns indicating commentary, interviews, or non-core-reporting content.
// Titles matching these are deprioritised as headline candidates so that a
// cleaner factual title from another source is preferred instead.
 ...
 *
 * Strategy:
 *   1. Filter out subjective/commentary titles (live updates, "my", "me", etc.).
 *   2. Restrict candidates to non-paywalled, high/medium confidence sources.
 *   3. Among those candidates, pick the title with the highest average
 *      Jaccard similarity to all other candidate titles — the title that
 *      best represents the "common thread" of the story.
 *   4. Falls back down the filter chain if the strict pool is empty.
 *
...
function pickBestHeadline(members) {
  ...
  // Helper: apply subjectivity filter; keep all if every title is subjective
  const noSubj = pool => {
    const f = pool.filter(m => !looksSubjective(m.title));
    return f.length ? f : pool;
  };

  // Candidate pool — progressively relaxed
  ...
  // Jaccard centroid: pick the title most representative of all candidates.
  // member.tokens is already a normalised Set<string> from buildClusters.
