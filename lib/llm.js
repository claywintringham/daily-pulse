...
export async function summarizeClusters(clusters) {
  ...
  try {
    ...
    // Parse "[id] summary text" lines.
    for (const line of content.split('\n')) {
      const m = line.match(/^\[([^\]]+)\]\s+(.+)/);
      if (!m) continue;
      const id   = m[1].trim();
      const text = m[2].trim();
      // Reject summaries that are too short (title echo or refusal).
      if (text.split(/\s+/).length < 35) continue;
      // Reject if the LLM itself signals the article content was off-topic.
