...
// Returns clusters with `headline` (neutral, adapted) and `summary` (40-75 words).
//
// Headline rules (enforced in prompt):
//   - Max 10 words, neutral factual tone
//   - NEVER use: "me", "my", "I", "live updates", "live blog", "opinion",
//                "analysis", "comment", "exclusive", "watch:"
//   - Adapted from the source titles — do not invent facts
//   - Do not include the outlet name
//
// Summary rules:
...
export async function summarizeClusters(clusters) {
  ...
  const items = clusters.map(c => ({
    id:       c.id,
    // Cluster headline — used as context for the story subject
    headline: decodeEntities(c.headline),
  ...
  try {
      ...
      messages: [
        {
          ...
          content:
            ...
            'For each story cluster you receive a current headline and a titles array ' +
            '(one title per cited source). Produce a new headline and a summary.\n\n' +
            'HEADLINE rules — max 10 words, neutral, factual:\n' +
            '  • Adapt from the source titles; do not invent facts.\n' +
            '  • Never use first-person words: "me", "my", "I".\n' +
            '  • Never use format labels: "live updates", "live blog", "live:", ' +
            '"opinion:", "analysis:", "comment:", "exclusive:", "watch:".\n' +
            '  • Do not include the outlet or journalist name.\n' +
            '  • Use plain nouns and verbs — no clickbait or emotion words.\n\n' +
            'SUMMARY rules — 40–75 words, factual, plain English:\n' +
            ...
            '  • Be specific: locations, actors, numbers when most sources agree.\n\n' +
            'Output ONLY valid JSON — an array of\n' +
            '{ "id": "...", "headline": "...", "summary": "..." }\n' +
            'objects, one per input cluster, same ids, no markdown fences.\n' +
            'Both headline and summary must be single lines with no line breaks.',
        },
        {
          role:    'user',
    ...
    // Sanitise first (handles literal newlines Gemini puts inside string values),
    // then extract the JSON array.
    const safe    = sanitiseForJson(content);
    const arrJson = extractJson(safe, '[', ']');
    const objJson = extractJson(safe, '{', '}');
    const arrPos  = arrJson ? safe.indexOf('[') : Infinity;
    const objPos  = objJson ? safe.indexOf('{') : Infinity;
  ...
  return clusters.map(c => {
    ...
    return {
      ...c,
      // Use LLM-synthesised headline if available; keep existing as fallback
      headline: r?.headline ? decodeEntities(r.headline) : decodeEntities(c.headline),
