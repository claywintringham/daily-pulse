...
// timeout_ms: per-attempt abort timeout (default 30 s; summarizeClusters uses 60 s).
async function gemini({ model = MODEL_SMART, messages, temperature = 0.3, max_tokens = 2048, timeout_ms = 30_000 }) {
    ...
    messages: [
      {
        ...
        content: 'Group these news articles by story. Articles about the same real-world event ' +
                 ...
                 'For each group, write a concise factual headline (10-15 words) synthesised from\n' +
                 'all the source titles in the group (there will be at least 2 titles, at most 5).\n' +
                 'Rules for the headline:\n' +
                 '- Declarative statement — not a question\n' +
                 '- Do NOT start with content-type labels: Graphic:, Photo:, Video:, Watch:, Listen:, Interactive:, Map:\n' +
                 '- Do NOT start with Opinion:, Analysis:, Comment:, Explainer:, Live:\n' +
                 '- Based only on the source titles provided — no outside knowledge\n' +
                 '- Include specific names, places, or figures if present in the titles\n\n' +
...
export async function summarizeClusters(clusters) {
  ...
  // Cap excerpt length and count per cluster to keep the total prompt size
  // manageable. gemini-2.5-flash is a thinking model — sending 5 × 6 000-char
  // excerpts across 9 clusters (270 000 chars) reliably exceeds the 30 s
  // timeout. 3 × 1 500 chars per cluster stays well under 60 000 chars total.
  ...
  const prompt = clusters.map((c, i) => {
    const excerpts = (c.articleExcerpts?.length)
      ? c.articleExcerpts
          .slice(0, EXCERPT_MAX_COUNT)
          .map((a, j) => `   Article ${j + 1}: ${a.slice(0, EXCERPT_MAX_CHARS)}`)
          .join('\n')
  ...
  try {
    const content = await gemini({
      model: MODEL_FAST,
      timeout_ms: 60_000, // larger budget: thinking model + multi-cluster batch
      messages: [
        {
          role: 'system',
          content:
            ...
            '- Write the best summary you can from whatever is provided. If content is limited, ' +
            'write a shorter summary (minimum 20 words). NEVER write a sentence explaining that ' +
            'there is insufficient information — always produce a summary, never a refusal.\n' +
            ...
            '- Some excerpts may contain website noise mixed in with real content — ignore any ' +
            'non-prose fragments and summarise only the actual news text. Examples of noise to ignore: ' +
            'social sharing labels (e.g. "x whatsapp-stroke copylink google"), URL fragments ' +
            '(e.g. ".dw.com/p/5CDqm"), advertisement markers ("Advertisement"), reading-time labels ' +
            '("2-MIN READ"), and bylines ("By [Author] Published On [Date]").\n\n' +
    ...
    // Catch any Gemini refusal / meta-comment instead of a real summary.
    // Patterns: "This story does not contain...", "The provided articles do not...",
    // "There is not enough information...", "I cannot provide a summary...", etc.
    const badSig = /\b(?:does not|doesn.t|do not|don.t|cannot|can.t|insufficient|not enough)\b[\s\S]{0,80}\b(?:contain|include|mention|information|detail|context|summarize|summary|provide)\b|\b(?:this story|the (?:article|content|excerpt|provided|story)|there is)\b[\s\S]{0,60}\b(?:does not|doesn.t|not enough|insufficient|cannot)\b/i;
    ...
    const kind = e.name === 'AbortError' ? 'timeout' : e.message.slice(0, 60);
    console.warn(`[llm] summarizeClusters error (${kind}):`, e.message);
  }
...
export async function translateToZh(items) {
  ...
  // ── Inner helper: translate a subset, returning parsed [{i, headline, summary}] ──
  async function geminiTranslate(subset) {
    // subset: [{origIdx, item}, ...]  — origIdx is the index in the full items array
    const input = subset.map(({ origIdx, item }) => ({
      i:        origIdx,
      ...
      messages: [
        {
          ...
          content:
            ...
            'IMPORTANT: You MUST include EVERY item in your response — never skip any.\n' +
            'Return ONLY a JSON array — no markdown, no extra text:\n' +
            '[{"i":0,"headline":"...","summary":"..."},...]',
        },
        { role: 'user', content: JSON.stringify(input) },
      ],
      // Higher ceiling so thinking-model reasoning tokens don't crowd out the output
      max_tokens: Math.max(8192, 500 * subset.length),
    });
    const safe = sanitiseForJson(content);
    // Gemini 2.5 Flash is a thinking model — its reasoning output often contains
    // square brackets (e.g. "[8 items]") before the actual JSON array.
    // Searching for "[{" instead of just "[" skips those false positives.
    let json = null;
    const arrayStart = safe.indexOf('[{');
    if (arrayStart !== -1) {
      json = extractJson(safe.slice(arrayStart), '[', ']');
    }
    if (!json) json = extractJson(safe, '[', ']');
    if (!json) throw new Error(`No JSON array in translation: ${content.slice(0, 120)}`);
    return JSON.parse(json); // [{i, headline, summary}, ...]
  }

  const byIdx = {};
  try {
    const all    = items.map((item, origIdx) => ({ origIdx, item }));
    const parsed = await geminiTranslate(all);
    parsed.forEach(t => { byIdx[t.i] = t; });

    // ── Retry any items Gemini dropped ────────────────────────────────────────
    const missing = items
      .map((item, origIdx) => ({ origIdx, item }))
      .filter(({ origIdx }) => !byIdx[origIdx]);

    if (missing.length > 0) {
      console.warn(`[llm] translateToZh: ${missing.length}/${items.length} items missing — retrying`);
      try {
        const retried = await geminiTranslate(missing);
        retried.forEach(t => { byIdx[t.i] = t; });
      } catch (retryErr) {
        console.warn('[llm] translateToZh retry failed:', retryErr.message);
      }
    }
  } catch (e) {
