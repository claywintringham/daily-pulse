...
export async function summarizeClusters(clusters) {
  ...
  try {
      ...
      messages: [
        {
          ...
          content:
            ...
            '• ANTI-TAUTOLOGY: Every summary MUST add at least one concrete fact ' +
            'that is not already stated in the headline — for example: a named actor ' +
            'or body, a specific action or decision, a location, a number, a cause, ' +
            'a consequence, a timeline, or an official response. ' +
            'Simply rephrasing the headline in different words is NOT acceptable.\n\n' +
    ...
    // Safety net: detect tautological summaries that merely rephrase the headline.
    // Compares non-trivial token overlap (overlap / min_size). A summary that
    // reuses >70% of the headline's content words adds no information.
    function isTautological(summary, headline) {
      ...
      return inter / Math.min(hs.size, ss.size) > 0.70;
    }
