...
function pickFallbackSummary(c) {
  // Prefer articleExcerpts array (plural) if available
  ...
  if (excerpts.length > 0) {
    ...
    if (first && first.length > 80 && !looksLikeNavJunk(first)) {
      const words = first.trim().split(/\s+/);
      if (words.length <= 75) return first.trim();
...
export async function summarizeClusters(clusters) {
  ...
  try {
      ...
      messages: [
        {
          ...
          content:
            ...
            '• Reflect what the MAJORITY of sources report. If only one source mentions ' +
    ...
    // reuses >65% of the headline's content words adds no information.
