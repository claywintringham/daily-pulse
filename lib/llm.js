...
export async function summarizeClusters(clusters) {
  ...
  // Pre-fill all summaries with null so missing entries stay null rather than
  // falling back to the headline — the frontend omits the paragraph if null.
  ...
  try {
    ...
    // summaries array stays all-null — no headline fallback
  }

  return clusters.map((c, i) => ({
    ...c,
    summary: summaries[i], // null if Gemini didn't produce one
  }));
}
