...
export async function editorialFilter(clusters) {
  ...
  try {
    ...
    if (s === -1 || e2 === -1) throw new Error('No JSON object in Gemini response');
    result = JSON.parse(content.slice(s, e2 + 1));
  } catch (e) {
...
export async function summarizeClusters(clusters) {
  ...
  try {
    ...
    if (arrStart !== -1 && (objStart === -1 || arrStart < objStart)) {
      summaries = JSON.parse(content.slice(arrStart, content.lastIndexOf(']') + 1));
    } else if (objStart !== -1) {
      // Gemini returned a single object instead of array — wrap it
      summaries = [JSON.parse(content.slice(objStart, content.lastIndexOf('}') + 1))];
    } else {
