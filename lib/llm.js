  ...
  try {
    ...
    for (const line of content.split('\n')) {
      ...
      if (m) {
        ...
        if (idx >= 0 && idx < clusters.length) summaries[idx] = m[2].trim();
      }
    }
  } catch (e) {
 ...
 * Picks the longest source article title that doesn't look like commentary
 * or an interview (avoids "my", "live updates", etc.). This is distinct from
 * the cluster headline — it's a raw article title from one of the cited
 * outlets, giving readers a different, more detailed line of text.
 */
function pickFallbackSummary(c) {
  const SKIP = [
    ...
    /^live:/i,
    /\blive:/i,        // catches "Middle East crisis live: ..." mid-title
    /\bwatch:/i, /\bexclusive:/i,
    /\?$/,             // question titles speculate rather than report
    /^day \d+\b/i,     // conflict day-counter titles ("Day 44 of ...")
  ];
  ...
  if (!titles.length) {
    // Everything is subjective — use any title as last resort
    return decodeEntities(c.members?.[0]?.title ?? c.headline);
  }

  // Prefer the longest title (most informative)
