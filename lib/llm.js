...
function pickFallbackSummary(c) {
  if (c.articleExcerpt && c.articleExcerpt.length > 80) {
    ...
    return sentenceEnd > 0 ? rough.slice(0, sentenceEnd + 1) : rough + '…';
  }
  const SKIP = [
    /\bmy\b/i, /\bme\b/i, /\bi [''']ve\b/i,
    /\bopinion[:\s]/i, /\bcomment[:\s]/i, /\banalysis[:\s]/i, /\binterview[:\s]/i,
    /\bexplainer[:\s]/i, /\blive updates?\b/i, /\blive blog\b/i,
    /^live:/i, /\blive:/i, /\bwatch:/i, /\bexclusive:/i, /\?$/, /^day \d+\b/i,
  ];
...
export async function summarizeClusters(clusters) {
  ...
  try {
    ...
    const parseLine = (id, text) => {
      ...
      if (badSig.test(text)) {
        console.warn('[llm] id %s: article mismatch — using title fallback', id);
