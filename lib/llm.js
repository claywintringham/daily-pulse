      ...
      messages: [
        {
          ...
          content: 'You are an editorial AI for a Hong Kong news digest. ' +
                   ...
                   'A HK outlet (RTHK, SCMP, HKFP) covering an international sports match or ' +
                   'foreign election does NOT make it a HK story — discard it from local.\n' +
      ...
      messages: [
        {
          ...
          content:
            ...
            '• CRITICAL: Reply ONLY in English. Do not translate or switch to any other language.\n\n' +
    ...
    for (const line of content.split('\n')) {
      ...
      if (m) {
        ...
        if (idx >= 0 && idx < clusters.length && text.split(/\s+/).length >= 35) {
          // Reject if the LLM itself signals the article is off-topic.
          // e.g. "The provided article does not contain information about..."
          // When detected, use title-only fallback so the digest shows real news.
          const badSig = /\bprovided (?:article|content)\b[\s\S]{0,80}\b(?:does not|doesn.t)\b[\s\S]{0,60}\b(?:contain|include|mention)\b/i;
          if (!badSig.test(text)) {
            summaries[idx] = text;
          } else {
            console.warn('[llm] story %d: article mismatch flagged by LLM, using title fallback', idx + 1);
            summaries[idx] = pickFallbackSummary({ ...clusters[idx], articleExcerpt: null });
          }
        }
      }
    }
