...
export async function summarizeClusters(clusters) {
  ...
  // Use simple sequential IDs ("0", "1", "2"…) instead of the internal cluster
  // IDs (which can look like "cluster-3-merged"). Gemini sometimes normalises
  // or drops the "-merged" suffix, which breaks the map lookup and causes every
  // merged cluster to fall back to its headline as the summary.
  const items = clusters.map((c, i) => ({
    id:       String(i),
    headline: decodeEntities(c.headline),
    // All source titles so Gemini can find the common thread
    titles:   [...new Set(c.members.map(m => decodeEntities(m.title)))],
  }));
      ...
      messages: [
        {
          ...
          content:
            ...
            'For each story cluster you receive a headline and a titles array ' +
            '(one title per cited source). Write a summary only.\n\n' +
            ...
            'Each summary must be a single line with no line breaks.',
        },
        {
          role:    'user',
  ...
  // Build lookup by sequential ID. Also keep the results array for a
  // position-based fallback in case Gemini returns results in order but
  // with slightly different id formatting.
  const map = {};
  for (const r of (results ?? [])) if (r?.id != null) map[String(r.id)] = r;

  return clusters.map((c, i) => {
    const r = map[String(i)] ?? results?.[i];
    return {
      ...c,
