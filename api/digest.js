...
export default async function handler(req, res) {
  ...
  try {
    ...
    // If the scraper (Pipeline 1) already enriched clusters, this is instant
    // (enrichWithArticleContent skips clusters that already have articleExcerpt).
    // On inline-scrape fallback, enrichment happens here without Firecrawl.
    const toEnrich = [
    ...
    if (needsEnrichment > 0) {
      console.log(`[digest] Enriching ${needsEnrichment} clusters without excerpts…`);
      await enrichWithArticleContent(toEnrich); // no Firecrawl in Pipeline 2
    } else {
      console.log('[digest] All clusters pre-enriched by scraper — skipping fetch');
    }
    ...
    const HEADLINE_SKIP = [
      ...
      /^the latest news from\b/i,                         // broadcaster promo headlines
    ];
    // Local-specific filter: exclude non-HK content that slips through the editorial
    // filter (e.g. RTHK/TVB sports feed, US financial news promos).
    const NON_HK_SPORTS_LOCAL = [
      ...
      /\bFox Business\b/i,                                // US financial news channel
      /\b(Wall Street|Main Street)\b.*\bon\s+\w/i,       // "X on Fox/CNN/…" show promos
    ];
